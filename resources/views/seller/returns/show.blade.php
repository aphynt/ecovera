    ...
    <div class="container-fluid">
        ...
        <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
            <div class="flex-grow-1">
                <h4 class="fs-18 fw-semibold m-0">Detail Pengajuan Pengembalian - Seller</h4>
            </div>
            <div class="mt-2 mt-sm-0">
                <a href="{{ route('seller.returns.index') }}" class="btn btn-secondary btn-sm">
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-4 border-success">
                    <div class="card-header bg-success-subtle">
                        <h5 class="card-title mb-0">
                            <i data-feather="map-pin" class="icon-xs me-1"></i> Alamat Pengembalian
                        </h5>
                    </div>
                    <div class="card-body">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <!-- Approve Form -->
                        <form method="POST" action="{{ route('seller.returns.approve', $return->uuid) }}" class="mb-3">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label">Alamat Pengembalian <span class="text-danger">*</span></label>
                                <textarea name="return_address" class="form-control" rows="3" placeholder="Masukkan alamat lengkap untuk pengembalian barang..." required></textarea>
                                <small class="text-muted">Buyer akan mengirim barang ke alamat ini</small>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Catatan Seller (opsional)</label>
                                <textarea name="seller_note" class="form-control" rows="2" placeholder="Tambahkan catatan untuk buyer..."></textarea>
                            </div>
                            <button type="submit" class="btn btn-success w-100" onclick="return confirm('Apakah Anda yakin ingin menyetujui pengembalian ini?')">
                                <i data-feather="check" class="icon-xs me-1"></i> Setujui Pengembalian
                            </button>
                        </form>
                        ...
                        <!-- Reject Form -->
                        <form method="POST" action="{{ route('seller.returns.reject', $return->uuid) }}">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label">Alasan Penolakan <span class="text-danger">*</span></label>
                                <textarea name="seller_note" class="form-control" rows="3" placeholder="Berikan alasan penolakan..." required></textarea>
                            </div>
                            <button type="submit" class="btn btn-danger w-100" onclick="return confirm('Apakah Anda yakin ingin menolak pengembalian ini?')">
                                <i data-feather="x" class="icon-xs me-1"></i> Tolak Pengembalian
                            </button>
                        </form>
                        ...
                        <div class="alert alert-info">
                            ...
                            Menunggu buyer mengirimkan barang kembali ke alamat yang telah diberikan.
                        </div>
                        <div class="mt-3">
                            <label class="text-muted small">Alamat Pengembalian</label>
                        ...
                        <div class="alert alert-warning">
                            ...
                            Barang sedang dalam pengiriman kembali.
                        </div>
                        @if($return->shipment)
                        <div class="mb-3">
                            <label class="text-muted small">Kurir</label>
                            <p class="fw-semibold">{{ $return->shipment->courier }}</p>
                        </div>
                        <div class="mb-3">
                            <label class="text-muted small">No. Resi</label>
                            <p class="fw-semibold">{{ $return->shipment->tracking_number ?? '-' }}</p>
                        </div>
                        @endif
                        <form method="POST" action="{{ route('seller.returns.mark-received', $return->uuid) }}" class="mt-3">
                            @csrf
                            <div class="mb-3">
                                ...
                                <select name="item_condition" class="form-select" required>
                                    ...
                                    <option value="missing">Tidak Lengkap</option>
                                </select>
                                <small class="text-muted">Pilih "Baik" untuk melanjutkan ke proses refund</small>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Catatan Penerimaan</label>
                                <textarea name="seller_note" class="form-control" rows="2" placeholder="Catatan saat menerima barang..."></textarea>
                            </div>
                            <button type="submit" class="btn btn-primary w-100" onclick="return confirm('Apakah Anda yakin barang sudah diterima?')">
                                <i data-feather="package" class="icon-xs me-1"></i> Tandai Barang Diterima
                            </button>
                        </form>
                        ...
                        <div class="alert alert-success">
                            ...
                            Barang telah diterima dan sesuai. Menunggu admin untuk memproses refund.
                        </div>

                        @elseif($return->return_status == 'refunded')
                        <div class="alert alert-success">
                            <i data-feather="check-circle" class="icon-xs me-1"></i>
                            Pengembalian telah selesai. Dana sudah dikembalikan ke buyer.
                        </div>
