                                    ...
                                    @foreach ($data['user'] as $user)
                                        ...
                                        : asset('logo/logo.png') }}" alt=""
                                                                        ...
                                                                        <td>
                                                                            @if ($user->role == 'admin')
                                                                                <span
                                                                                    class="badge bg-secondary-subtle text-success">{{ ucfirst($user->role) }}</span>
                                                                            @elseif ($user->role == 'seller')
                                                                                <span
                                                                                    class="badge bg-secondary-subtle text-info">{{ ucfirst($user->role) }}</span>
                                                                            @else
                                                                                <span
                                                                                    class="badge bg-secondary-subtle text-secondary">{{ ucfirst($user->role) }}</span>
                                                                            @endif
                                                                        </td>
                                                                        <td>{{ $user->phone }}</td>
                                                                        <td>
                                                                        ...
                                                                        <td>{{ \Carbon\Carbon::parse($user->created_at)->locale('id')->translatedFormat('d F Y H:i') }}
                                                                        </td>
                                                                        <td class="text-end">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.btn-edit-user').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                // If user is Seller, disable Buyer option
                if (role === 'seller') {
                    for (let i = 0; i < roleOptions.length; i++) {
                        if (roleOptions[i].value === 'buyer') {
